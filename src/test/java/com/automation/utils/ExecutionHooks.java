...
public class ExecutionHooks {
    ...
    public void afterScenario(ExecutionContext context){
        if(context.getCurrentScenario().getIsFailing()){
            ...
            String filePath = GlobalConstants.NETWORK_LOGS_PATH + scenarioName + DateUtils.getCurrentTimestamp() + ".txt";
            //Take screenshot
            Gauge.captureScreenshot();
            //Capture Network Logs
            DriverFactory.getInstance().captureNetworkLogs(filePath);
            //Attach network log file
            Gauge.writeMessage("Network Logs: "+"<a href='../../../../"+filePath+"' download>Download</a>");
        }
    }
